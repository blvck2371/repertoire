...
- **Phase 9 :** Backups : MinIO + CronJob déployés automatiquement. Ajouter `minio-repertoire` et `minio-api-repertoire` dans DuckDNS pour accès HTTPS. Voir [docs/BACKUP-RESTORE.md](docs/BACKUP-RESTORE.md)
...
**Prérequis :** ajouter `minio-repertoire` et `minio-api-repertoire` dans DuckDNS (même IP que repertoire-app). La console a besoin de l’API pour fonctionner.
