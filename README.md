...
- **Phase 9 :** Backups : MinIO (Bitnami) + CronJob déployés automatiquement. Ajouter `minio-repertoire` dans DuckDNS pour accès HTTPS. Voir [docs/BACKUP-RESTORE.md](docs/BACKUP-RESTORE.md)
...
**Prérequis :** ajouter `minio-repertoire` dans DuckDNS (même IP que repertoire-app). Chart Bitnami – la console se connecte à l’API en interne.
